...
import co.com.juan.tcr.dao.IProveedorAppDao;
...
public class ProveedorAppLogic implements IProveedorAppLogic {
	...
	/**
	 * DAO injected by Spring that manages ProveedorApp entities
	 * 
	 */
	@Autowired
	...
	/**
	 * Logic injected by Spring that manages ProveedorApp entities
	 * 
	 */
	@Override
	...
	public List<ProveedorApp> getProveedorApp() {
		...
		List<ProveedorApp> list = new ArrayList<>();
	...
	public List<ProveedorApp> findPageProveedorApp(String sortColumnName, boolean sortAscending, int startRow,
			int maxResults) {
		List<ProveedorApp> entity = null;

		try {
			entity = proveedorAppDao.findPage(sortColumnName, sortAscending, startRow, maxResults);
		} catch (Exception e) {
			throw new EntityException().new FindingException(Constant.ENTITY_NAME);
		}
